...
class EnemyFleet extends Factory {
	...
	makeItem(type, startX, startY, direction) {
		switch(type) {
			case "standard" :
				const photos = ['nivi', 'rushi', 'jenniviv', 'matt', 'hann'];
				const random = Math.floor(Math.random() * photos.length);
				const enemy = new EnemyShip(this.game, this, this.bombFactory, startX, startY, direction, `enemy${this.makeCounter}`, photos[random]);
				return enemy;
